...
func TestRotateSessionTokensQueryShape(t *testing.T) {
	...
	if !strings.Contains(rotateSessionTokens, "refresh_token_hash = $1") {
		t.Fatal("expected new refresh token hash update in SET clause")
	}
	if !strings.Contains(rotateSessionTokens, "access_token_jti = $2") {
		t.Fatal("expected access token jti update in SET clause")
	}
	if !strings.Contains(rotateSessionTokens, "WHERE id = $3") {
		t.Fatal("expected session id match in WHERE clause")
	}
	if !strings.Contains(rotateSessionTokens, "refresh_token_hash = $4") {
