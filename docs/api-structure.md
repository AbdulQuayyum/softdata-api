...
Domain and API-facing models kept separate from sqlc-generated persistence structs. `healthcare.go` defines the unified health-facility snapshot model, medical laboratory accreditation snapshot model, NHIA HMO and SSHIA organisation models, and the NHIA active-accredited healthcare provider snapshot model, with model tests validating each dataset and reconciliation package.

### `internal/repository/`

Repository interfaces plus PostgreSQL, Redis, and file-backed implementations. The file repository includes lazy, synchronized, validated loading for education snapshots, indexed paginated school access, and source-verified healthcare and emergency repositories with immutable filter indexes. Healthcare repository loading validates the committed health-facility, medical laboratory accreditation, and NHIA accredited Health Maintenance Organisation snapshots and geography references where applicable but does not load reconciliation partitions. The emergency contact package exposes generated public data, schema, metadata, reconciliation, model, file-backed repository and service, list and detail handlers, validators, OpenAPI/Postman contract, production routes, standard public emergency middleware, stable usage templates, and bounded startup verification for 5 official institutional emergency-contact records. Startup verification and later HTTP requests share the same repository cache. Runtime code reads only the public dataset JSON and does not call, message, or operationally test emergency contacts, execute the generator, or load reconciliation metadata. The NEMA Zonal, Territorial and Operation offices package contains a dated 17-record snapshot of the narrower source-described office category, not a complete all-office register or live operational guarantee. It exposes repository/service support, list/detail handlers, validators, OpenAPI and Postman contracts, production GET routes for `/v1/emergency/nema-zonal-territorial-operation-offices` and `/v1/emergency/nema-zonal-territorial-operation-offices/{office_id}`, standard public Emergency middleware, optional API-key identification, public rate limiting, usage tracking with `dataset_group=emergency`, stable usage templates, bounded startup verification, and a shared repository cache between startup verification and HTTP requests. The FRSC Zonal Commands package currently stops at the dataset foundation boundary: generated public data, schema, metadata, reconciliation, model and generator tests for a dated 12-record official “Zonal Commands” snapshot. It excludes sector commands, unit commands, outposts, driver-licence centres, contact details, commanders, staff names and operational status, and repository/service, HTTP, OpenAPI, Postman and startup verification integration are deferred. The medical laboratory accreditation implementation exposes the static dated MLSCN Accreditation Service snapshot through paginated repository and service contracts, public list and detail endpoints, filtering by `state_id`, `accreditation_status` and `search`, production route registration, standard public middleware, OpenAPI documentation, and bounded startup verification. It contains 30 records, with 26 marked `accredited` and 4 marked `expired`; it is not a complete live medical-laboratory licensing or premises register, does not establish that every Nigerian laboratory is licensed, contains no personal practitioner information, and does not implement websites or logos. The NHIA accredited HMO package exposes the file-backed repository and service, list and detail handlers, validators, OpenAPI contract, production routes, standard public healthcare middleware, stable usage templates, and bounded startup verification for a dated NHIA accreditation snapshot of 94 Health Maintenance Organisation rows, all marked `accredited`, with filters limited to `accreditation_status`, `hmo_id`, and organisation-name `search`. Startup verification and later HTTP requests share the same repository cache. The NHIA HMO contract intentionally omits contact, state, location, website and logo enrichment, performs no runtime NHIA network calls, and does not load reconciliation metadata at runtime. It is not a complete live licensing, registration or operational-status register, and SSHIAs remain outside this dataset. The NHIA SSHIA package exposes generated public data, schema, metadata, reconciliation, model, file-backed repository and service, list and detail handlers, validators, OpenAPI HTTP contract, production routes, standard public healthcare middleware, stable usage templates, and bounded startup verification for 37 NHIA-listed State Social Health Insurance Agencies. Startup verification and later HTTP requests share the same repository cache. The SSHIA contract omits personal director/contact information, addresses, websites, logos and status claims, performs no runtime NHIA network calls, and does not load reconciliation metadata at runtime. It is a dated NHIA-listed snapshot, not a complete live accreditation, licensing, registration or operational-status register. The NHIA active-accredited healthcare provider package exposes generated public data, schema, metadata, reconciliation, model, generator tests, file-backed repository and service, list and detail handlers, validators, OpenAPI HTTP contract, production routes, standard public healthcare middleware, stable usage templates, and bounded startup verification for 6,536 provider records. Startup verification and later HTTP requests share the same repository cache. It retains only provider code, provider name, facility type, country code and listing status from the NHIA HCP source and discards addresses during extraction because they may contain contact details or possible personal names. Runtime code reads only the public dataset JSON and does not call NHIA, execute the generator, or load reconciliation metadata. Deterministic reconciliation and generation preserve all committed healthcare snapshots.
...
HTTP router construction, route registration, public and authenticated route groups, route cataloging, stable usage templates, and embedded flag, commercial-bank, and regulated-finance logo serving. Healthcare production routes are registered as GET-only collection and detail routes under `/v1/healthcare/health-facilities`, `/v1/healthcare/medical-laboratory-accreditations`, `/v1/healthcare/nhia-accredited-health-maintenance-organisations`, `/v1/healthcare/nhia-state-social-health-insurance-agencies`, and `/v1/healthcare/nhia-active-accredited-healthcare-providers`. Emergency contact production routes are registered as GET-only collection and detail routes under `/v1/emergency/emergency-service-contacts`, use `dataset_group=emergency`, and track stable usage templates without literal contact IDs.
